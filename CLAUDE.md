# jarvis — Package Source Reference

Technical reference for AI assistants modifying the jarvis NPM package source code.
...
All event handler logic, API routes, library code, and core functionality lives in the **npm package** (`api/`, `lib/`, `config/`, `bin/`). This is what users import when they `import ... from 'jarvis/...'`.

The `templates/` directory contains **only files that get scaffolded into user projects** via `npx jarvis init`. Templates are for user-editable configuration and thin wiring — things users are expected to customize or override. Never add core logic to templates.
...
├── config/
│   ├── index.js                # withjarvis() Next.js config wrapper
...
| `config/index.js` | `withjarvis()` Next.js config wrapper |
| `config/instrumentation.js` | `register()` startup hook (loads .env, validates AUTH_SECRET, init DB, starts crons) |
| `bin/cli.js` | CLI entry point (`jarvis init`, `setup`, `reset`, `diff`, etc.) |
...
|--------|--------|---------|
| `jarvis/api` | `api/index.js` | `GET` and `POST` route handlers — re-exported by the user's catch-all route |
| `jarvis/config` | `config/index.js` | `withjarvis()` — wraps the user's Next.js config to mark server-only packages as external |
| `jarvis/instrumentation` | `config/instrumentation.js` | `register()` — Next.js instrumentation hook that loads `.env` and starts cron jobs on server start |
| `jarvis/auth` | `lib/auth/index.js` | Auth helpers (`auth()`, `getPageAuthState()`) |
| `jarvis/auth/actions` | `lib/auth/actions.js` | Server action for admin setup (`setupAdmin()`) |
| `jarvis/chat` | `lib/chat/components/index.js` | Chat UI components |
| `jarvis/chat/actions` | `lib/chat/actions.js` | Server actions for chats, notifications, and swarm |
| `jarvis/chat/api` | `lib/chat/api.js` | Dedicated chat streaming route handler (session auth) |
| `jarvis/db` | `lib/db/index.js` | Database access |
| `jarvis/middleware` | `lib/auth/middleware.js` | Auth middleware |
...
SQLite via Drizzle ORM at `data/jarvis.sqlite` (override with `DATABASE_PATH`). Auto-initialized on server start.

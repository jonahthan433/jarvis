...
Commands:
  init                              Scaffold a new jarvis project
...
async function init() {
  ...
  // Guard: warn if the directory is not empty (unless it's an existing jarvis project)
  ...
  if (!fs.existsSync(pkgPath)) {
    ...
    const jarvisDep = version.includes('-') ? version : '^1.0.0';
    const pkg = {
      name: dirName,
      private: true,
      scripts: {
        ...
        start: 'next start',
        setup: 'jarvis setup',
        'setup-telegram': 'jarvis setup-telegram',
        'reset-auth': 'jarvis reset-auth',
      },
      dependencies: {
        jarvis: jarvisDep,
        next: '^15.5.12',
  ...
  if (changed.length > 0) {
    ...
    console.log('  This may be from your edits, or from a jarvis update.\n');
    for (const file of changed) {
      console.log(`    ${file}`);
    }
    console.log('\n  To view differences:  npx jarvis diff <file>');
    console.log('  To reset to default:  npx jarvis reset <file>');
  }
  ...
  const jarvisPkg = JSON.parse(fs.readFileSync(path.join(packageDir, 'package.json'), 'utf8'));
  const version = jarvisPkg.version;

  if (!fs.existsSync(envPath)) {
    // Seed .env for new projects
    const authSecret = randomBytes(32).toString('base64');
    const seedEnv = `# jarvis Configuration
...
`;
    ...
    console.log(`  Created .env (AUTH_SECRET, jarvis_VERSION=${version})`);
  } else {
    // Update jarvis_VERSION in existing .env
    try {
      let envContent = fs.readFileSync(envPath, 'utf8');
      if (envContent.match(/^jarvis_VERSION=.*/m)) {
        envContent = envContent.replace(/^jarvis_VERSION=.*/m, `jarvis_VERSION=${version}`);
      } else {
        envContent = envContent.trimEnd() + `\njarvis_VERSION=${version}\n`;
      }
      fs.writeFileSync(envPath, envContent);
      console.log(`  Updated jarvis_VERSION to ${version}`);
    } catch {}
  }
...
function reset(filePath) {
  ...
  if (!filePath) {
    ...
    console.log('\nUsage: jarvis reset <file>');
    console.log('Example: jarvis reset config/SOUL.md\n');
    return;
  }
  ...
  if (!fs.existsSync(src)) {
    ...
    console.log('Run "jarvis reset" to see available templates.\n');
    process.exit(1);
  }
...
function diff(filePath) {
  ...
  if (!filePath) {
    ...
    console.log('\nUsage: jarvis diff <file>');
    console.log('Example: jarvis diff config/SOUL.md\n');
    return;
  }
  ...
  if (!fs.existsSync(dest)) {
    ...
    console.log(`Run "jarvis reset ${filePath}" to create it.\n`);
    return;
  }
  ...
  try {
    ...
    console.log(`\n  To reset: jarvis reset ${filePath}\n`);
  }
}
 ...
 * Supports piped stdin (e.g. echo "val" | jarvis set-var KEY).
 */
...
async function setAgentSecret(key, value) {
  if (!key) {
    console.error('\n  Usage: jarvis set-agent-secret <KEY> [VALUE]\n');
    console.error('  Example: jarvis set-agent-secret ANTHROPIC_API_KEY\n');
    process.exit(1);
  }
...
async function setAgentLlmSecret(key, value) {
  if (!key) {
    console.error('\n  Usage: jarvis set-agent-llm-secret <KEY> [VALUE]\n');
    console.error('  Example: jarvis set-agent-llm-secret BRAVE_API_KEY\n');
    process.exit(1);
  }
...
async function setVar(key, value) {
  if (!key) {
    console.error('\n  Usage: jarvis set-var <KEY> [VALUE]\n');
    console.error('  Example: jarvis set-var LLM_MODEL claude-sonnet-4-5-20250929\n');
    process.exit(1);
  }

...
| `DATABASE_PATH` | Override SQLite database location (default: `data/jarvis.sqlite`) | No |
| `jarvis_VERSION` | Package version for Docker image tags (auto-set by setup) | No |
...
Set via `npx jarvis set-agent-secret` and `npx jarvis set-agent-llm-secret`. Each value is stored exactly as provided.

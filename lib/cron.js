...
function getInstalledVersion() {
  const pkgPath = path.join(process.cwd(), 'node_modules', 'jarvis', 'package.json');
...
async function fetchAndStoreReleaseNotes(target) {
  try {
    const ghRes = await fetch(
      `https://api.github.com/repos/JonathanBrightAmpwera/jarvis/releases/tags/v${target}`
    );
    if (!ghRes.ok) return;
    ...
    console.warn(`[version check] failed to fetch release notes: ${err.message}`);
  }
}

/**
 * Check npm registry for a newer version of jarvis.
 */
  ...
  try {
    ...
    if (isPrerelease(installed)) {
      ...
      const results = await Promise.allSettled([
        fetch('https://registry.npmjs.org/jarvis/latest'),
        fetch('https://registry.npmjs.org/jarvis/beta'),
      ]);
      ...
      const res = await fetch('https://registry.npmjs.org/jarvis/latest');
      if (!res.ok) {

...
2. **Suggest setting it up now** so the skill can be tested in the same job:
   - Run: `npx jarvis set-agent-llm-secret <KEY_NAME> <value>`
...
> ```
> npx jarvis set-agent-llm-secret SLACK_WEBHOOK_URL <your-url>
> ```
...
> 4. Test: run `post.sh "test message from jarvis"` and verify successful delivery. Fix any issues before committing.
